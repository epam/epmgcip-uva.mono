...
import { editEvent, isCreateFormDirty } from 'src/utils/editEvent';
import { validateEventValues } from 'src/utils/validateEventValues';
import { v4 as uuidv4 } from 'uuid';
import css from './EditEventPage.module.sass';
...
export const EditEventPage = () => {
  ...
  const handleSubmit = (event: { preventDefault: () => void }) => {
    ...
    if (Object.keys(validation).length === 0) {
      ...
      editEvent(newEvent, image as File).then(() => {
        handleEditEvent();
      });
    }
  };
  ...
  return (
    isEditorHasPermissions && (
      <div className={css.editEventWrapper}>
        <div className={css.editEventTitle}>{translation.editEvent}</div>
        <form className={css.editEventForm} onSubmit={handleSubmit}>
          <ImageLoader
            setImage={setImage}
          ...
          <DatePicker
            ...
            max={eventEndDate}
            required
            labelClassName={css.editEventPadding}
          ...
          <div className={css.buttonsPanel}>
            <Button onClick={onLeave} className={`${css.editEventButton} ${css.backButton}`}>
              {translation.back}
            </Button>
            <Button
              className={`${css.editEventButton} ${css.submitButton}`}
              id="edit-event-submit"
            >
              {isCreating ? <Loader size={'12px'} /> : translation.save}
            </Button>
          </div>
        </form>
